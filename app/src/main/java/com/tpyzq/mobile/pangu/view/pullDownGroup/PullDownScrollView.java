    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (ev.getAction() == MotionEvent.ACTION_MOVE) {
            ...
            if (!isElastic && canScroll()
                ...
                isElastic = true;
                startY = (int) ev.getY();
                LogUtil.i(TAG, "在move时候记录下位置startY:" + startY);
                return true;
            }
                    ...
                    if (state != IPullDownElastic.REFRESHING && isElastic) {
                        ...
                        if (state == IPullDownElastic.RELEASE_To_REFRESH) {
                            if (((tempY - startY) / RATIO < headContentHeight)
                                ...
                                LogUtil.i(TAG, "由松开刷新状态转变到下拉刷新状态");
                            } else if (tempY - startY <= 0) {
                                state = IPullDownElastic.DONE;
                                changeHeaderViewByState(state, false);
                                LogUtil.i(TAG, "由松开刷新状态转变到done状态");
                            }
                        }
                        if (state == IPullDownElastic.DONE) {
                            if (tempY - startY > 0) {
                        ...
                        if (state == IPullDownElastic.PULL_To_REFRESH) {
                            ...
                            if ((tempY - startY) / RATIO >= headContentHeight) {
                                ...
                                changeHeaderViewByState(state, false);
                                LogUtil.i(TAG, "由done或者下拉刷新状态转变到松开刷新");
                            } else if (tempY - startY <= 0) {
                                state = IPullDownElastic.DONE;
                                changeHeaderViewByState(state, false);
                                LogUtil.i(TAG, "由DOne或者下拉刷新状态转变到done状态");
                            }
                        }
                        if (tempY - startY > 0) {
...
//        }
    ...
    public void finishRefresh(String text) {
        ...
        changeHeaderViewByState(state,false);
        LogUtil.i(TAG, "==>执行了=====finishRefresh "+ text);
    ...
    private boolean canScroll() {
        ...
        if (getChildCount() > 1) {
            ...
            if (childView instanceof AbsListView) {
                View view = ((AbsListView) childView).getChildAt(0);
                if (view == null) {
                    return false;
                }
