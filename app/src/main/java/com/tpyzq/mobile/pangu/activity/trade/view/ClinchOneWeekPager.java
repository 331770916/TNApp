 ...
 * 成交    一周内   标签
 */
public class ClinchOneWeekPager extends BaseSearchPager {
    private String TAG = "ClinchOneWeekPager";
    ...
    public ClinchOneWeekPager(Context context) {
        super(context);
    }
                ...
                if (mClinchListView.isShownHeader()) {
                    ...
                    new AsyncTask<Void, Void, Void>() {
                        ...
                        protected Void doInBackground(Void... params) {
                            try {
                                Thread.sleep(1500);
                                handler.sendEmptyMessage(1); // 向Handler发送消息,更新UI
                            } catch (InterruptedException e) {
                                e.printStackTrace();
                            }
                            return null;
