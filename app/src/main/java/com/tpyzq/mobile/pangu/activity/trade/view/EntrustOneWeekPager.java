 ...
 * 委托  一周内
 */
public class EntrustOneWeekPager extends BaseSearchPager {
    private String TAG = "EntrustOneWeekPager";
    private PullToRefreshListView mOneWeekListView;
    ...
    public EntrustOneWeekPager(Context context) {
        super(context);
    }

    @Override
    public void setView() {
        mOneWeekListView = (PullToRefreshListView) rootView.findViewById(R.id.TodayListView);
        ...
        mOneWeekListView.setAdapter(mAdapter);
        mOneWeekListView.setEmptyView(iv_isEmpty);
        Update();
    }
...
//                ToastUtils.centreshow(mContext, "网络异常");
                    ...
                    if ("0".equals(jsonObject.getString("code"))) {
                        ...
                        if (data != null && data.length() > 0) {
                            ...
                            for (int i = 0; i < data.length(); i++) {
                                ...
                                _bean.setTime(data.getJSONObject(i).getString("ORDER_TIME"));
     ...
     */
    private void Update() {
        mOneWeekListView.setOnRefreshListener(new PullToRefreshBase.OnRefreshListener<ListView>() {
            @Override
            public void onRefresh(PullToRefreshBase<ListView> refreshView) {
                if (mOneWeekListView.isShownHeader()) {
                    //设置头布局样式文字
                    mOneWeekListView.getLoadingLayoutProxy().setRefreshingLabel("正在刷新");
                    mOneWeekListView.getLoadingLayoutProxy().setPullLabel("下拉刷新数据");
                    mOneWeekListView.getLoadingLayoutProxy().setReleaseLabel("释放开始刷新");
                    ...
                    new AsyncTask<Void, Void, Void>() {
                        ...
                        protected void onPostExecute(Void result) {
                            ...
                            //将下拉视图收起
                            mOneWeekListView.onRefreshComplete();
                        }
                    }.execute();
                } else if (mOneWeekListView.isShownFooter()) {
                    //设置尾布局样式文字
                    mOneWeekListView.getLoadingLayoutProxy().setRefreshingLabel("正在刷新");
                    mOneWeekListView.getLoadingLayoutProxy().setPullLabel("上拉加载数据");
                    mOneWeekListView.getLoadingLayoutProxy().setReleaseLabel("释放开始刷新");
