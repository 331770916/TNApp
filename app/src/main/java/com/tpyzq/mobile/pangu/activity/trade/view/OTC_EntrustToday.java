...
/**
 * 作者：刘泽鹏 on 2016/9/6 19:31
 * OTC 委托查询  当日
 */
public class OTC_EntrustToday extends BasePager  {

    private String TAG = "OTC_EntrustToday";
    private PullToRefreshListView mListView;
    private ArrayList<HashMap<String,String>> list;
    private OTC_EntrustTodayAdapter adapter;
    private ImageView OTCEntrustKong;

    public OTC_EntrustToday(Context context) {
        super(context);
    }
    ...
    private void getData() {
        ...
        NetWorkUtil.getInstence().okHttpForPostString(TAG, ConstantUtil.URL_JY, map1, new StringCallback() {
            ...
            public void onResponse(String response, int id) {
                ...
                /**
                Gson gson = new Gson();
                java.lang.reflect.Type type = new TypeToken<OTC_EntrustTodayBean>() {}.getType();
                OTC_EntrustTodayBean todayBean = gson.fromJson(response, type);
                String code = todayBean.getCode();
                List<OTC_EntrustTodayBean.DataBean> data = todayBean.getData();
                if (code.equals("-6")) {
                    Intent intent = new Intent(mContext, TransactionLoginActivity.class);
                    mContext.startActivity(intent);
                }else
                ...
                if(code.equals("0") && data !=null){
                    for(int i=0;i<data.size();i++){
                        HashMap<String,String> map = new HashMap<String, String>();
                        OTC_EntrustTodayBean.DataBean dataBean = data.get(i);
                        ...
                        map.put("tv_state",dataBean.getENTRUST_STATUS_NAME());
