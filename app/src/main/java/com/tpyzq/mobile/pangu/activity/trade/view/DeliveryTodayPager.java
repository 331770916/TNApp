 ...
 * 交割单  今日 标签
 */
public class DeliveryTodayPager extends BaseSearchPager {
    private final String TAG = "DeliveryTodayPager";
    private DeliveryListViewAdapter mListViewAdapter;
    private PullToRefreshListView mDeliveryTodayListView;
    ...
    public DeliveryTodayPager(Context context) {
        super(context);
    }
     ...
     */
    private void Update() {
        mDeliveryTodayListView.setOnRefreshListener(new PullToRefreshBase.OnRefreshListener<ListView>() {
            @Override
            public void onRefresh(PullToRefreshBase<ListView> refreshView) {
                if (mDeliveryTodayListView.isShownHeader()) {
                    //设置头布局样式文字
                    mDeliveryTodayListView.getLoadingLayoutProxy().setRefreshingLabel("正在刷新");
                    mDeliveryTodayListView.getLoadingLayoutProxy().setPullLabel("下拉刷新数据");
                    mDeliveryTodayListView.getLoadingLayoutProxy().setReleaseLabel("释放开始刷新");
                    ...
                    new AsyncTask<Void, Void, Void>() {
                        ...
                        protected void onPostExecute(Void result) {
                            ...
                            //将下拉视图收起
                            mDeliveryTodayListView.onRefreshComplete();
                        }
                    }.execute();
                } else if (mDeliveryTodayListView.isShownFooter()) {
                    //设置尾布局样式文字
                    mDeliveryTodayListView.getLoadingLayoutProxy().setRefreshingLabel("正在刷新");
                    mDeliveryTodayListView.getLoadingLayoutProxy().setPullLabel("上拉加载数据");
                    mDeliveryTodayListView.getLoadingLayoutProxy().setReleaseLabel("释放开始刷新");
