 ...
 * 委托 一月内
 */
public class EntrustInAMonthPager extends BaseSearchPager {
    private String TAG = "EntrustInAMonthPager";
    private PullToRefreshListView mInamonthListView;
    ...
    public EntrustInAMonthPager(Context context) {
        super(context);
    }

    @Override
    public void setView() {
        mInamonthListView = (PullToRefreshListView) rootView.findViewById(R.id.TodayListView);
        ...
        mInamonthListView.setAdapter(mAdapter);
     ...
     */
    private void Update() {
        mInamonthListView.setOnRefreshListener(new PullToRefreshBase.OnRefreshListener<ListView>() {
            @Override
            public void onRefresh(PullToRefreshBase<ListView> refreshView) {
                if (mInamonthListView.isShownHeader()) {
                    //设置头布局样式文字
                    mInamonthListView.getLoadingLayoutProxy().setRefreshingLabel("正在刷新");
                    mInamonthListView.getLoadingLayoutProxy().setPullLabel("下拉刷新数据");
                    mInamonthListView.getLoadingLayoutProxy().setReleaseLabel("释放开始刷新");
                    ...
                    new AsyncTask<Void, Void, Void>() {
                        ...
                        protected void onPostExecute(Void result) {
                            ...
                            //将下拉视图收起
                            mInamonthListView.onRefreshComplete();
                        }
                    }.execute();
                } else if (mInamonthListView.isShownFooter()) {
                    //设置尾布局样式文字
                    mInamonthListView.getLoadingLayoutProxy().setRefreshingLabel("正在刷新");
                    mInamonthListView.getLoadingLayoutProxy().setPullLabel("上拉加载数据");
                    mInamonthListView.getLoadingLayoutProxy().setReleaseLabel("释放开始刷新");
