...
import com.tpyzq.mobile.pangu.activity.home.managerMoney.view.chartView.chart.MyFundMarkView;
 ...
 * 货币基金折线图
 */
public class MoneyFundChartView extends MoneyFundBaseView implements RadioGroup.OnCheckedChangeListener, ICallbackResult {

    private RadioGroup mRadioGroup;
    private MyLineChart mMyLineChart;
    private MyFundMarkView mMyMarkerView;
    ...
    private static final String TAG ="MoneyFundChartView";


    public MoneyFundChartView(Activity activity, Object object) {
    ...
    public void initView(View view, Activity activity, Object object) {
        ...
        mProgress = (ProgressBar) view.findViewById(R.id.fundChartProgress);
        mRadioGroup = (RadioGroup) view.findViewById(R.id.moneyFundChartBottomLayout);
        mRadioGroup.setOnCheckedChangeListener(this);

        mMyLineChart = (MyLineChart) view.findViewById(R.id.moneyFundLineChart);
    ...
    public void getResult(Object result, String tag) {
        ...
        try {
            ...
            for (Map<String, String> item : chartDatasList) {
                ...
                String latestweeklyyield = item.get("latestweeklyyield");

                if (mMounthFlag) {
                    mMounthXdates.add(endTime);
                    mMounthValues.add(Float.parseFloat(latestweeklyyield));
                } else if (mQuarterFlag) {
                    mQuarterXdates.add(endTime);
                    mQuarterValues.add(Float.parseFloat(latestweeklyyield));
                } else if (mHelfYearFlag) {
                    mHelfYearXdates.add(endTime);
                    mHelfYearValues.add(Float.parseFloat(latestweeklyyield));
                } else if (mOneYearFlag) {
                    mOneYearXdates.add(endTime);
                    mOneYearValues.add(Float.parseFloat(latestweeklyyield));
                }
    ...
    private void initChart(MyLineChart myLineChart) {
        ...
        axisLeftLine.setLabelCount(8, true);
        ...
        mMyMarkerView = new MyFundMarkView(CustomApplication.getContext(), R.layout.mark_mistakefundchart);
    ...
    private void setChartData(MoneyFundFundChartView.TypeXLabels typeXLabels, ArrayList<String> strDates, ArrayList<Float> values) {
        ...
        LineDataSet lineDataSet = new LineDataSet(lineChartEntries, "7日年化收益率");

        lineDataSet.setDrawValues(false);
        lineDataSet.setCircleRadius(0);
//        lineDataSet.setColor(Color.parseColor("#daecff"));
        lineDataSet.setColor(ContextCompat.getColor(CustomApplication.getContext(), R.color.red));
    ...
    public int getLayoutId() {
        return R.layout.view_moneyfund_chart;
    }

    @Override
    public void onCheckedChanged(RadioGroup group, int checkedId) {
        switch (checkedId) {
            case R.id.moneyFundChartBtn1:
                mMounthFlag = true;
                mQuarterFlag = false;
                mHelfYearFlag = false;
                mOneYearFlag = false;
